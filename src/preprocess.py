...
def parse_records(payload: dict) -> list[dict]:
    rows = payload.get(config.DATASET, {}).get("row", [])
    return [
        {
            "year": int(row["YEAR"]),
            "month": int(row["MON"]),
...
def print_problem_2_1(df: pd.DataFrame) -> None:
    ...
    print("문제 2-1. 수집한 JSON 데이터를 pandas DataFrame으로 변환하고 기본 정보를 출력하시오.")
    print("[결과]")
...
def main() -> None:
    ...
    if not raw_dir.exists():
        raise FileNotFoundError("원본 데이터가 존재하지 않습니다. 먼저 fetch 스크립트를 실행하세요.")
    ...
    output_path = Path(config.PROCESSED_DATA_DIR) / "energy_usage_personal.csv"
